...
class ContextAnalysis():
    ...
    def run_context_analysis(self, models, contexts, metrics, baseline_models, return_all=False, numPictures=None):
        ...
        if return_all:
            ...
            summary = np.zeros((num_cxt, num_mod, 8))
        ...
        return summary, ['nss mean', 'sim mean', 'ig mean', 'auc mean', 'nss std', 'sim std', 'ig std', 'auc std']
    ...
    def run_dataset_analysis(self, metrics, baseline, return_all):
        ...
        if return_all:
            ...
            return np.nanmean(nss), np.nanmean(sim), np.nanmean(ig), np.nanmean(auc), np.nanstd(nss), np.nanstd(sim), np.nanstd(ig), np.nanstd(auc)
